...
def generate_train_plot(y_train_encoded, y_train_pred):
    ...
    plt.scatter(y_train_encoded, y_train_pred, color='blue', alpha=0.5,label='Predicciones')
    plt.plot([y_train_encoded.min(), y_train_encoded.max()], [y_train_encoded.min(), y_train_encoded.max()], 'r--',  label='Linea de Prediccion')
...
def home():
    ...
    return render_template('indexx.html', metrics=metrics, plot=plot_base64)
